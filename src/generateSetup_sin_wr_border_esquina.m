function [gainOverNoisedB,R,pilotIndex,D,D_small,APpositions,UEpositions,distances] = generateSetup_sin_wr_border_esquina(squareLength,L,K,N,tau_p,nbrOfSetups,seed,ASD_varphi,ASD_theta)
...
for n = 1:nbrOfSetups
    ...
    for k = 1:K 
        if mod(k, 5) <= 1
            % Generar dentro del cuadrado en la esquina superior izquierda (40% de las veces)
            % Tamaño del lado del cuadrado es 10% de squareLength
            side_length = 0.1 * squareLength;
            x = rand * side_length;
            y = squareLength - side_length + rand * side_length;
        
            % Almacenamiento de la posición del elemento como un número complejo
            UEposition = x + 1i * y;
        else
            % Generar dentro del área cuadrada (60% de las veces)
            % Generar una posición aleatoria dentro del área cuadrada
            x = (rand * 0.6 + 0.2) * squareLength;
            y = (rand * 0.6 + 0.2) * squareLength;
        ...
        [distanceAPstoUE,whichpos] = min(abs(APpositions - repmat(UEposition,size(APpositions))),[],2);
        ...
        if k-1>0
            ...
            for i = 1:k-1
                ...
                % shortestDistances(i) = min(abs(UEposition));
            end
        ...
        for l = 1:L
            ...
            angletoUE_varphi = angle(UEpositions(k)-APpositions(l,whichpos(l))); %azimuth angle
